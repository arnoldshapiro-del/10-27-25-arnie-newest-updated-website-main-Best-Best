// Anger Assessment Implementation
const questions = [
  "I am quick tempered",
  ...
  "I feel like breaking things when I'm frustrated",
  "I get angry when people are unfair"
];
...
function renderForm() {
  ...
  let html = '<p><strong>Please indicate how often each statement applies to you:</strong></p>';
...
function calculateResults() {
  ...
  if (score <= 12) {
    level = "Low anger expression";
    color = "#10b981";
    recommendation = "Your responses suggest low anger expression. Continue practicing healthy emotional regulation.";
  } else if (score <= 24) {
    level = "Moderate anger expression";
    color = "#f59e0b";
    recommendation = "Your responses suggest moderate anger expression. Consider learning anger management techniques.";
  } else {
    level = "High anger expression";
    color = "#ef4444";
    recommendation = "Your responses suggest high anger expression. Recommend consultation with a mental health professional for anger management strategies.";
  }
  ...
  resultDiv.innerHTML = `
    <div class="card" style="border-left: 4px solid ${color};">
      <h2>Your Results</h2>
      <p><strong>Total Score: ${score}/36</strong></p>
...
function downloadPDF() {
  ...
  const doc = new jsPDF();
  const title = "Anger Assessment Results";
  ...
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('Total Score: ' + currentScore + '/36', 105, yPos + 12, { align: 'center' });
  doc.setFontSize(12);
  ...
  doc.save('Anger_Assessment_Results_' + new Date().toISOString().split('T')[0] + '.pdf');
}
